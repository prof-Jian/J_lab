...
class ROLLOUT:
    ...
    def get_reward(self, sentences, rollout_num, dis, current_k=0): #seqGAN：rewards = rollout_func.get_reward(target, cfg.rollout_num, self.dis)
        """
        get reward via Monte Carlo search
        :param sentences: size of batch_size * max_seq_len
        :param rollout_num: 我猜是论文中哪个N
        :param dis:
        ...
        with torch.no_grad():
            batch_size = sentences.size(0)
            rewards = torch.zeros([rollout_num * self.max_seq_len, batch_size]).float() # 16*20,64
            if self.gpu:
            ...
            for i in range(rollout_num):
                for given_num in range(1, self.max_seq_len + 1):
                    ...
                    out = dis.forward(samples) # batch_size * 2
                    out = F.softmax(out, dim=-1) # batch_size * 2
                    reward = out[:, current_k + 1] # (batch_size,)
                    rewards[idx] = reward
                    # print("rewards.shape = ",rewards.shape,"\n reward.shape = ",reward.shape) ->[320,64]  [64]
                    idx += 1

        # rewards = torch.mean(rewards, dim=0)
        rewards = torch.mean(rewards.view(batch_size, self.max_seq_len, rollout_num), dim=-1) #我觉得这个view可能并不能达到想要的结果
        print("rewards.shape = ",rewards.shape)
        exit()
        return rewards # batch_size*max_seq_len
