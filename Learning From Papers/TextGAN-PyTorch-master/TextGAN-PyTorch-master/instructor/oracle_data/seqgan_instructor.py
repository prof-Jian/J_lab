...
class SeqGANInstructor(BasicInstructor):
    ...
    def _run(self):
        ...
        if not cfg.gen_pretrain:
            ...
            self.pretrain_generator(cfg.MLE_train_epoch) #pretrain G用交叉熵作为loss函数
        ...
        if not cfg.dis_pretrain:
            ...
            self.train_discriminator(cfg.d_step, cfg.d_epoch)  # 同样用交叉熵作为loss函数
            if cfg.if_save and not cfg.if_test:
    ...
    def adv_train_generator(self, g_step):
        ...
        for step in range(g_step):
            inp, target = GenDataIter.prepare(self.gen.sample(cfg.batch_size, cfg.batch_size), gpu=cfg.CUDA)#把G sample出来的句子切割成错位的
    ...
    def train_discriminator(self, d_step, d_epoch, phase='MLE'):
        ...
        Samples are drawn d_step times, and the discriminator is trained for d_epoch.
        """
