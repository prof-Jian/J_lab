...
class LeakGANInstructor(BasicInstructor):
    ...
    def _run(self):
        for inter_num in range(cfg.inter_epoch):
            ...
            if self.sig.pre_sig:
                # ===DISCRIMINATOR PRE-TRAINING=== 因为结构特殊，区别seqGAN，先训练D
                if not cfg.dis_pretrain:
                    self.log.info('Starting Discriminator Training...')
                    self.train_discriminator(cfg.d_step, cfg.d_epoch)
                    if cfg.if_save and not cfg.if_test:
                        torch.save(self.dis.state_dict(), cfg.pretrained_dis_path)
                        print('Save pre-trained discriminator: {}'.format(cfg.pretrained_dis_path)) #默认这句不执行
    ...
    def pretrain_generator(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            if self.sig.pre_sig:
                ...
                # ===Train===
                for i, data in enumerate(self.oracle_data.loader): #这是真数据？
