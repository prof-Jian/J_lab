...
class LeakGAN_G(nn.Module):
    ...
    def forward(self, idx, inp, work_hidden, mana_hidden, feature, real_goal, no_log=False, train=False):
        """
        Embeds input and sample on token at a time (seq_len = 1) #那这里为什么不用cell
        ...
        :param feature: 1 * batch_size * total_num_filters, feature of current sentence[from Discreminator????]
        ...
        # Manager
        mana_out, mana_hidden = self.manager(feature, mana_hidden)  # mana_out: 1 * batch_size * hidden_dim 第二个不应该是(h_0,c_0)吗
    ...
    # in pretrain_dis: neg_samples = self.gen.sample(cfg.samples_num, cfg.batch_size, self.dis)
    def sample(self, num_samples, batch_size, dis, start_letter=cfg.start_letter, train=False):
        ...
        samples = samples[:num_samples, :] # cut to num_samples
    ...
    def forward_leakgan(self, sentences, dis, if_sample, no_log=False, start_letter=cfg.start_letter, train=False):
        # in pretrain_dis: epoch sentences = fake_sentences = torch.zeros((batch_size, self.max_seq_len))
        """
