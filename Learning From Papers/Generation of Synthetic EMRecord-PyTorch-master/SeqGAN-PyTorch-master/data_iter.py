...
import numpy as np
import torch
class GenDataIter(object): #为什么不用torch.DataSets类
    """ Toy data iter to load digits"""
    def __init__(self, data_file, batch_size):
        super(GenDataIter, self).__init__()
        self.batch_size = batch_size
        self.data_lis = self.read_file(data_file) #data_list现在就是一串串数字
    ...
    def next(self):
        ...
        d = torch.LongTensor(np.asarray(d, dtype='int64')) # 否则无法embding， batch_size * 20
        data = torch.cat([torch.zeros(self.batch_size, 1).long(), d], dim=1) #每句前面加0
        target = torch.cat([d, torch.zeros(self.batch_size, 1).long()], dim=1) # 每句后面加0
